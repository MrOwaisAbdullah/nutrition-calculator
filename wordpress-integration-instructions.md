# WordPress Integration Instructions for Nutrition Calculator

## Overview
This document provides instructions for integrating the nutrition calculator into WordPress using the WPCode plugin with two separate snippets: a global data snippet and a UI shortcode snippet.

## Implementation Strategy

### Snippet 1: Global Data Snippet (Site-wide)
- This snippet will be available throughout your entire WordPress site
- Contains the nutrition data array that can be accessed by other snippets
- Runs automatically on every page load

### Snippet 2: UI Shortcode Snippet 
- This snippet contains the HTML, CSS, and JavaScript for the calculator interface
- Only loads when the shortcode is used on a specific page/post
- References the data from the global snippet

## WPCode Plugin Implementation

### Step 1: Create the Global Data Snippet
1. Go to **WPCode** → **Snippets** → **Add New**
2. Add a title like "Nutrition Calculator - Data"
3. Set the snippet type to **PHP Snippet**
4. Set the **Usage** to **Entire Site** (this makes it available site-wide)
...
### Step 2: Create the UI Shortcode Snippet
1. Go to **WPCode** → **Snippets** → **Add New**
2. Add a title like "Nutrition Calculator - UI"
3. Set the snippet type to **PHP Snippet**
4. Set the **Usage** to **Shortcode** 
5. Set the shortcode name to `nutrition-calculator` (or your preferred name)
6. Paste the following content:

```php
<?php
function nutrition_calculator_shortcode() {
    ob_start();
    ?>
    ...
    (function() {
      ...
      // Use the globally available nutrition data
...
## How the Two Snippets Connect

### Connection Mechanism
- The global data snippet (set to "Entire Site") creates the PHP variable `$nutrition_items` available throughout your WordPress site
- The shortcode snippet accesses this variable directly when converting it to JSON for JavaScript with `json_encode($nutrition_items)`
- When you use the shortcode `[nutrition-calculator]` on a page, it renders the UI with access to the globally available nutrition data

### Important Considerations
- Both snippets must be active for the calculator to work properly
- The global data snippet must be loaded before the shortcode snippet on the page
- All existing functionality (search, filtering by category, meal planning, etc.) will continue to work as before
- The dynamic category generation will work automatically based on the categories included in the data

## Adding the Calculator to a Page

1. Edit the page where you want the nutrition calculator to appear
2. Add the shortcode `[nutrition-calculator]` at the desired location
3. Update or publish the page

## Maintenance

- To update nutritional information, simply modify the data in the global data snippet
- The UI will automatically reflect any changes to the nutrition data
- No changes are needed to the shortcode snippet when updating data

## Troubleshooting

If you encounter issues:
1. Ensure both snippets are activated in WPCode
2. Check that the global data snippet is set to load site-wide
3. Verify the shortcode is correctly placed on the page as `[nutrition-calculator]`
4. Check browser console for any JavaScript errors
5. Test that the NUTRITION_DATA variable is properly populated in browser developer tools